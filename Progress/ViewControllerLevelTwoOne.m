...
@interface ViewControllerLevelTwoOne ()
...
@implementation ViewControllerLevelTwoOne
...
//{
//  UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"Stage 2.1" message:@"Swipe right any one from the 5 choices to match from 3 reference images" preferredStyle:UIAlertControllerStyleAlert];
...
- (void) initImageArrays
{
  ...
  self.lvl_referentImagePathStringSavedArr = [[NSMutableArray alloc] initWithCapacity:LVL_TWO_ONE__IMAGE_REFERENT_TOTAL_COUNT];
  self.lvl_choiceImagePathStringSavedArr   = [[NSMutableArray alloc] initWithCapacity:LVL_TWO_ONE__CHOICE_BUTTON_TOTAL_COUNT];
}
...
- (void) initCompnentsDefaults
{
  ...
  self.label_levelName.text = @"LEVEL 2.1";
...
{
  if (self.lvl_totalChoiceCount <= LVL_TWO_ONE__PUZZLES_PER_LEVEL_MAX)
  {
...
-(IBAction)dragOut: (id)sender withEvent: (UIEvent *) event
{
  ...
  if (dragCount >= LVL_TWO_ONE__IMAGE_REFERENT_TOTAL_COUNT)
  {
    dragCount = 0;
    //prevButtonStr = @"";
...
{
  for (UInt8 i = 0; i < LVL_TWO_ONE__IMAGE_REFERENT_TOTAL_COUNT; i++)
  {
    self.lvl_referentImagePathStringSavedArr[i] = [self getRandomReferentImagePathAsString];
  }
  
  for (UInt8 i = 0; i < LVL_TWO_ONE__CHOICE_BUTTON_TOTAL_COUNT; i++)
  {
    if (i < LVL_TWO_ONE__IMAGE_REFERENT_TOTAL_COUNT)
    {
...
- (void) loadNextView
{
  [self performSegueWithIdentifier:@"segue_levelTwoOneToLevelScoreSheet" sender:self];
...
- (void) prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
  if ([segue.identifier isEqualToString:@"segue_levelTwoOneToLevelScoreSheet"])
  {
    ...
    vc.lvl_scoreSheet_levelNumberPlayed     = LVL_PLAYED__TWO_ONE;

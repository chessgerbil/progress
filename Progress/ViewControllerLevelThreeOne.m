...
@interface ViewControllerLevelThreeOne ()
...
@implementation ViewControllerLevelThreeOne
...
//{
//  UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"Stage 3.1" message:@"Select the the sequence after the image disapper from the choices" preferredStyle:UIAlertControllerStyleAlert];
...
- (void) componentsActivate:(BOOL)status
{
  ...
  //DONGcode - all choices are hidden
...
- (void) initImageArrays
{
  ...
  self.lvl_referentImagePathStringSavedArr = [[NSMutableArray alloc] initWithCapacity:LVL_THREE_ONE__IMAGE_REFERENT_TOTAL_COUNT];
  self.lvl_choiceImagePathStringSavedArr   = [[NSMutableArray alloc] initWithCapacity:LVL_THREE_ONE__CHOICE_BUTTON_TOTAL_COUNT];
}
...
- (void) initCompnentsDefaults
{
  ...
  self.label_levelName.text = @"LEVEL 3.1";
...
{
  if (self.lvl_totalChoiceCount <= LVL_THREE_ONE__PUZZLES_PER_LEVEL_MAX)
  {
...
-(IBAction)dragOut: (id)sender withEvent: (UIEvent *) event
{
  ...
  if (dragCount >= LVL_THREE_ONE__IMAGE_REFERENT_TOTAL_COUNT)
  {
    dragCount = 0;
    //prevButtonStr = @"";
...
{
  for (UInt8 i = 0; i < LVL_THREE_ONE__IMAGE_REFERENT_TOTAL_COUNT; i++)
  {
    self.lvl_referentImagePathStringSavedArr[i] = [self getRandomReferentImagePathAsString];
  }
  
  for (UInt8 i = 0; i < LVL_THREE_ONE__CHOICE_BUTTON_TOTAL_COUNT; i++)
  {
    if (i < LVL_THREE_ONE__IMAGE_REFERENT_TOTAL_COUNT)
    {
  ...
  */
  ...
  //DONGcode - sequential disappearing of referents - ref2 disappears 0.75s later
  dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.70 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
    self.imageView_referent_2.image = nil;
  });

  //DONGcode - sequential disappearing of referents - 0.25s later, choices appear
  dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.95 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
  ...
  // update choice image 0 + DONGcode hides the image
  UIImage *choice_0 = [UIImage imageNamed:[self.lvl_choiceImagePathStringSavedArr objectAtIndex:0]];
  [self.button_choice_0 setBackgroundImage:choice_0 forState:UIControlStateNormal];
  self.button_choice_0.hidden = YES;

  // update choice image 1 + DONGcode hides the image
  UIImage *choice_1 = [UIImage imageNamed:[self.lvl_choiceImagePathStringSavedArr objectAtIndex:1]];
  [self.button_choice_1 setBackgroundImage:choice_1 forState:UIControlStateNormal];
  self.button_choice_1.hidden = YES;
  
  // update choice image 2 + DONGcode hides the image
  UIImage *choice_2 = [UIImage imageNamed:[self.lvl_choiceImagePathStringSavedArr objectAtIndex:2]];
  [self.button_choice_2 setBackgroundImage:choice_2 forState:UIControlStateNormal];
  self.button_choice_2.hidden = YES;
  
  // update choice image 3 + DONGcode hides the image
  UIImage *choice_3 = [UIImage imageNamed:[self.lvl_choiceImagePathStringSavedArr objectAtIndex:3]];
  [self.button_choice_3 setBackgroundImage:choice_3 forState:UIControlStateNormal];
  self.button_choice_3.hidden = YES;
  
  // update choice image 4 + DONGcode hides the image
  UIImage *choice_4 = [UIImage imageNamed:[self.lvl_choiceImagePathStringSavedArr objectAtIndex:4]];
  [self.button_choice_4 setBackgroundImage:choice_4 forState:UIControlStateNormal];
  self.button_choice_4.hidden = YES;

  // update choice image 5 + DONGcode hides the image
  UIImage *choice_5 = [UIImage imageNamed:[self.lvl_choiceImagePathStringSavedArr objectAtIndex:5]];
  [self.button_choice_5 setBackgroundImage:choice_5 forState:UIControlStateNormal];
  self.button_choice_5.hidden = YES;
 
  // update choice image 6 + DONGcode hides the image
  UIImage *choice_6 = [UIImage imageNamed:[self.lvl_choiceImagePathStringSavedArr objectAtIndex:6]];
  [self.button_choice_6 setBackgroundImage:choice_6 forState:UIControlStateNormal];
  self.button_choice_6.hidden = YES;

  // update choice image 7 + DONGcode hides the image
  UIImage *choice_7 = [UIImage imageNamed:[self.lvl_choiceImagePathStringSavedArr objectAtIndex:7]];
  [self.button_choice_7 setBackgroundImage:choice_7 forState:UIControlStateNormal];
  self.button_choice_7.hidden = YES;
...
- (void) loadNextView
{
  [self performSegueWithIdentifier:@"segue_levelThreeOneToLevelScoreSheet" sender:self];
...
- (void) prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
  if ([segue.identifier isEqualToString:@"segue_levelThreeOneToLevelScoreSheet"])
  {
    ...
    vc.lvl_scoreSheet_levelNumberPlayed     = LVL_PLAYED__THREE_ONE;
